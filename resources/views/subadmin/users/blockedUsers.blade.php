        ...
        $(document).ready(function () {
            ...
            $(window).scroll(function() {
                ...
                if (isBottom && currentscrollHeight < scrollHeight) {
                    page++;
                    //alert('calling...');
                    var myurl = ENDPOINT + '/subadmin/blocked-users-get?page=' + page
                    getData(myurl);
            ...
            function getBlckedUsers(){
                ...
                $.ajax({
                    type: "POST",
                    url: "{{route('subadmin.blocked.users.get')}}",
                    data: {
                        '_token': "{{csrf_token()}}",
                        'type' : 1,
                    ...
                    success: function (data) {
                        ...
                        $('.show_blcket_usr').empty();
                        if (data.count_ses > 0){
                            $('.show_blcket_usr').append(data.view);
                        }
                        $('.show_animation').hide();
                    }
                });
