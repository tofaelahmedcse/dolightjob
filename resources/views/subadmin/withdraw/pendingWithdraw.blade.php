        ...
        $(document).ready(function () {
            ...
            $(window).scroll(function() {
                ...
                if (isBottom && currentscrollHeight < scrollHeight) {
                    page++;
                    //alert('calling...');
                    var myurl = ENDPOINT + '/subadmin/pending-withdraw-get?page=' + page
                    getData(myurl);
            ...
            function getPenWithdraw(){
                ...
                $.ajax({
                    type: "POST",
                    url: "{{route('subadmin.pending.withdraw.get')}}",
                    data: {
                        '_token': "{{csrf_token()}}",
                        'type' : 1,
                    ...
                    success: function (data) {
                        ...
                        $('.show_pen_with').empty();
                        if (data.count_ses > 0){
                            $('.show_pen_with').append(data.view);
                        }
                        $('.show_animation').hide();
                    }
                });
